...
class ProjectType extends Base
{
     ...
     */
    public $table = 'istheweb_iscorporate_project_types';
    ...
    public $rules = [
        'name' => 'required',
        'slug' => 'required|between:3,64|unique:istheweb_iscorporate_project_types',
        'code' => 'unique:istheweb_iscorporate_project_types',
    ];
    ...
    public $hasOne = [];
    public $hasMany = [
        'children' => ['Istheweb\IsCorporate\Models\ProjectType', 'key' => 'parent_id'],
    ];
    public $belongsTo = [
        'parent' => ['Istheweb\IsCorporate\Models\ProjectType', 'key' => 'parent_id'],
    ];
    ...
    public static function resolveMenuItem($item, $url, $theme)
    {
        ...
        if ($item->type == 'project-type') {
            ...
            $pageUrl = self::getProjectTypePageUrl($item->cmsPage, $category, $theme);
            if (!$pageUrl)
                return;
            ...
            if ($item->nesting) {
                ...
                $iterator = function($categories) use (&$iterator, &$item, &$theme, $url) {
                    ...
                    foreach ($categories as $category) {
                        ...
                        $branchItem = [];
                        $branchItem['url'] = self::getProjectTypePageUrl($item->cmsPage, $category, $theme);
        ...
        elseif ($item->type == 'all-project-types') {
            $result = [
                'items' => []
            ];
            ...
            foreach ($categories as $category) {
                $categoryItem = [
                    'title' => $category->name,
                    'url'   => self::getProjectTypePageUrl($item->cmsPage, $category, $theme),
     ...
     */
    protected static function getProjectTypePageUrl($pageCode, $category, $theme)
    {
